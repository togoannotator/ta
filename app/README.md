...
※ docker / docker-compose コマンドの先頭に sudo を付けていますが、他OS環境またはdockerコマンドが実行可能なユーザで実施の際は、sudoを外してください。

## ■コンテナ化前のnginx, togoannotatorプロセスの停止手順

コンテナ化する前のnginxやTogoAnnotatorアプリがホスト上で動作している場合に、停止する手順。
...
## ■プロキシ環境下で実行するための準備
※ 実行するホストからインターネットへのアクセスにプロキシが不要な場合は本項目はスキップしてください。
