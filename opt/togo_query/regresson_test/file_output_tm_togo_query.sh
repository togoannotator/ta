...
curl -sS --noproxy localhost -X GET "http://localhost:19200/${INDEX_NAME}/_search" -H 'Content-Type: application/json' -d@json.txt > result.txt
if [ -z "$8" ]; then
echo "===== All results ====="
cat result.txt | jq
echo ""
...
cat result.txt | jq '.aggregations.tags.buckets[].top_tag_hits.hits.hits[] | {"_score": ._score,"_source": ._source}' >> all_result.txt
